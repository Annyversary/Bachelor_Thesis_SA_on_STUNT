...
train_command_template = "python main.py --mode protonet --model mlp --dataset {dataset} --seed {seed} --outer_steps {steps} --num_shots 5 --num_shots_test {num_shots_test} --num_ways {num_ways}"

# Default command template for resuming training
resume_command_template = "python main.py --mode protonet --model mlp --dataset {dataset} --resume_path {resume_path} --seed {seed} --outer_steps {steps} --num_shots 5 --num_shots_test {num_shots_test} --num_ways {num_ways}"

# Default command template for evaluation
eval_command_template = ("python eval.py --data_name {dataset} --shot_num 5 --seed {seed} "
                         "--load_path {load_path}")
...
for dataset in datasets:
    ...
    for step in training_steps:
        ...
        # List to store numeric results for this step (for calculating average and standard deviation)
        numeric_results = []

        # Loop over all seeds
        for seed in seeds:
            # Set the log directory for the current seed
            log_dir = os.path.join("logs", f"{current_date}_{dataset}_mlp_protonet_{num_ways}way_5shot_{num_shots_test}query_seed{seed}")
            ...
            if step == training_steps[0]:
                ...
                train_command = resume_command_template.format(dataset=dataset, resume_path=resume_path, seed=seed, steps=step, num_ways=num_ways, num_shots_test=num_shots_test)
